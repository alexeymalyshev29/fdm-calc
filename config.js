...
var CONFIG = {
  ...
  brand: {
    ...
    appDesc:      'Калькулятор себестоимости FDM 3D печати — филамент, амортизация, электричество, оператор',
  },
  ...
  defaults: {
    ...
    maintenance:        5,      // ₽/ч   — fallback для принтеров без поля maintenance
    defect:             0,      // %     — брак по умолчанию для новых столов
  },

  // ── СУШИЛКИ ──────────────────────────────────────────────────
  // kw — мощность потребления (кВт), 0 = сушилки нет
  dryers: [
  ...
  // ── ПРИНТЕРЫ ─────────────────────────────────────────────────
  // cost          — стоимость принтера (₽), для расчёта амортизации
  // kw            — мощность при печати (кВт)
  // dryerId       — id сушилки из списка выше, привязанной к принтеру
  // maintenance   — стоимость обслуживания (₽/ч): смазка, сопла, расходники.
  //                 Задаётся индивидуально для каждого принтера.
  //                 Если не указано — используется defaults.maintenance как fallback.
  printers: [
  ...
  // color      — HEX-цвет для визуального отображения в интерфейсе
  filaments: [
